...
void http_conn::init(int sockfd, const sockaddr_in &addr) {
    ...
    // 避免 TIME_WAIT 状态，实际是否在应用时保留，看情况
    int reuse = 1;
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text) {
    if (text[0] == '\0') {  // 空行。表示头部解析完毕
        if (m_method == HEAD) { return GET_REQUEST; }

        // 有消息体，需要读取 m_content_length 字节的消息体
...
// 分析目标文件属性。如果存在，则使用 mmap 将文件映射到 m_file_address
...
bool http_conn::write() {
    ...
    while (1) {
        ...
        if (temp <= -1) {
            // 如果写缓冲区中没有空间，等待下一轮EPOLLOUT事件，虽然在此期间服务器无法
            // 立即接收同一个客户地下一个请求，但是可以保证连接地完整性
            if (errno == EAGAIN) {
        ...
        if (bytes_to_send <= bytes_have_send) {
            // 根据 m_linger 关联地 Connection 字段，决定是否立即关闭连接
            unmap();
            if (m_linger) {
                init();
                modfd(m_epollfd, m_sockfd, EPOLLIN);
